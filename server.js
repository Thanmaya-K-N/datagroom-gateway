...
// Attach the authentication middleware to all routes except /login and /rag
app.use(/^(?!\/login|\/rag).*$/, authenticate);
...
// RAG endpoints
const ragController = require('./rag/rag.controller');
...
(async () => {
    try {
        await dbAbstraction.hello();
        console.log("Started DG server..... Find the logs in ./datagroom.log");
    } catch (error) {
        console.log("Started DG server (DB pending)..... Find the logs in ./datagroom.log");
        logger.error(error, "Database connection failed during startup - server will continue");
    }
})();
